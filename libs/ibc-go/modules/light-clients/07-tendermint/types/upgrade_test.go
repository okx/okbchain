...
import (
	...
	upgradetypes "github.com/okx/brczero/libs/ibc-go/testing/simapp/types"
)
...
func (suite *TendermintTestSuite) TestVerifyUpgrade() {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setup: func() {
				cc := suite.chainB.GetContext()
				hh := cc.BlockHeight()
				// upgrade Height is at next block
				lastHeight = clienttypes.NewHeight(0, uint64(hh+1))
		...
		},
		{
			...
			setup: func() {
				cc := suite.chainB.GetContext()
				hh := cc.BlockHeight()
				upgradedHeight := clienttypes.NewHeight(0, uint64(hh+2))
...

		{
			...
			setup: func() {
				...
				cc := suite.chainB.GetContext()
				hh := cc.BlockHeight()
				lastHeight = clienttypes.NewHeight(0, uint64(hh+int64(10)))
		...
		},
		{
			...
			setup: func() {
				...
				cc := suite.chainB.GetContext()
				hh := cc.BlockHeight()
				lastHeight = clienttypes.NewHeight(0, uint64(hh+100))
