...
import (
	...
	fee "github.com/okx/brczero/libs/ibc-go/modules/apps/29-fee"
...
func (suite *InterchainAccountsTestSuite) TestOnChanOpenInit() {
	...
	for _, tc := range testCases {
		...
		suite.Run(tc.name, func() {
			...
			portCap := suite.chainA.GetSimApp().IBCKeeper.V2Keeper.PortKeeper.BindPort(suite.chainA.GetContext(), portID)
			...
			chanCap, err := suite.chainA.GetSimApp().GetScopedIBCKeeper().NewCapability(suite.chainA.GetContext(), host.ChannelCapabilityPath(path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID))
			suite.Require().NoError(err)
...
func (suite *InterchainAccountsTestSuite) TestChanOpenTry() {
	...
	msg := channeltypes.NewMsgChannelOpenTry(path.EndpointA.ChannelConfig.PortID, "",
		TestVersion, channeltypes.ORDERED,
		[]string{path.EndpointA.ConnectionID},
		path.EndpointB.ChannelConfig.PortID,
		path.EndpointB.ChannelID, TestVersion,
		proofInit, proofHeight, icatypes.ModuleName)
	handler := suite.chainA.GetSimApp().MsgServiceRouter().HandlerWithMsg(msg)
	...
	counterparty := channeltypes.NewCounterparty(path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID)
	chanCap, found := suite.chainA.GetSimApp().GetScopedIBCKeeper().GetCapability(suite.chainA.GetContext(), host.ChannelCapabilityPath(path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID))
