...
func (ms *MptStore) openSnapshot() error {
	...
	var err error
	ms.snaps, err = snapshot.NewCustom(snap.GetDiskDB(), ms.db.TrieDB(), 256, ms.originalRoot, false, true, recovery, ms.retriever)
	if err != nil {
...
func (ms *MptStore) updateSnapStorages(addr common.Address, key, bz []byte) {
	...
	addrHash := mpttypes.Keccak256HashWithSyncPool(addr.Bytes())
...
func (ms *MptStore) flattenPersistSnapshot() error {
	...
	return fmt.Errorf("snapshot circle not persist")
}
