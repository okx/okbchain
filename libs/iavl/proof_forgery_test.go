...
func TestProofForgery(t *testing.T) {
	...
	r := rand.New(source)
	cacheSize := 0
	tree, err := NewMutableTreeWithOpts(db.NewMemDB(), cacheSize, nil)
	...
	assert.Panics(t, func() { proof2.ComputeRootHash() }, "roothash must be empty if both left and right are set")
	assert.Panics(t, func() { proof2.Verify(nil) }, "should not verify")
	...
	assert.Panics(t, func() { proof2.Verify(rootHashValid) }, "should not verify different root hash")
