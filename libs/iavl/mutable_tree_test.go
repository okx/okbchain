...
import (
	...
	"github.com/okx/brczero/libs/iavl/mock"
...
func prepareTree2Version(t *testing.T) *MutableTree {
	mdb := db.NewMemDB()
	...
	_, ver, _, err := tree.SaveVersion(false)
...
func TestFastStorageReUpgradeProtection_NoForceUpgrade_Success(t *testing.T) {
	...
	// rIterMock is used to get the latest version from disk. We are mocking that rIterMock returns latestTreeVersion from disk
	rIterMock.EXPECT().Valid().Return(true).Times(2)
	rIterMock.EXPECT().Key().Return(rootKeyFormat.Key(latestTreeVersion)).Times(2)
...
func TestFastStorageReUpgradeProtection_ForceUpgradeFirstTime_NoForceSecondTime_Success(t *testing.T) {
	...
	batchMock.EXPECT().Delete(fastKeyFormat.Key(fastNodeKeyToDelete)).Return().Times(1)
	batchMock.EXPECT().Set(metadataKeyFormat.Key([]byte(storageVersionKey)), updatedExpectedStorageVersion).Return().Times(1)
	batchMock.EXPECT().Write().Return(nil).Times(2)
	batchMock.EXPECT().Close().Return().Times(2)
	...
	buf.Grow(amino.VarintSize(int64(latestFastStorageVersionOnDisk)) + amino.ByteSliceSize([]byte(testValue)))
	err := amino.EncodeVarint(&buf, int64(latestFastStorageVersionOnDisk))
	require.NoError(t, err)
	err = amino.EncodeByteSlice(&buf, []byte(testValue))
	require.NoError(t, err)
