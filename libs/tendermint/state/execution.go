...
func execBlockOnProxyApp(context *executionTask) (*ABCIResponses, error) {
	...
	for realTx := range realTxCh {
		if realTx != nil {
			...
			proxyAppConn.DeliverTxAsync(abci.RequestDeliverTx{Tx: txm[count].GetTx(), TxType: txm[count].GetTxType()})
		}
...
func preDeliverRoutine(proxyAppConn proxy.AppConnConsensus, txs types.TxWithMetas, realTxCh chan<- abci.TxEssentials, stopedCh <-chan struct{}) {
	for _, tx := range txs {
