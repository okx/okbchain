...
func (suite *KeeperTestSuite) TestProposal_ProxyContractRedirect() {
	denom1 := "testdenom1"
	externalContract := ethcmn.BigToAddress(big.NewInt(2))
	addr1 := ethcmn.BytesToAddress([]byte{0x0}).Bytes()

	proposal := types.NewProxyContractRedirectProposal(
		"default title",
		"default description",
		denom1,
		0,
...
func (suite *KeeperTestSuite) TestProposal_ContractTemplateProposal() {
	addr1 := ethcmn.BytesToAddress([]byte{0x0}).Bytes()
