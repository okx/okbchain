			...
			postcheck: func(ctx *sdk.Context, stateDB *types.CommitStateDB) {
				...
				suite.app.AccountKeeper.IterateAccounts(*ctx, func(account authexported.Account) bool {
					if account.GetAddress().Equals(sdk.AccAddress(suite.updateAddr.Bytes())) {
						suite.Require().Equal(sdk.NewCoins(sdk.NewCoin("okb", sdk.NewDec(2))), account.GetCoins())
					} else if account.GetAddress().Equals(sdk.AccAddress(suite.insertAddr.Bytes())) {
						suite.Require().Equal(sdk.NewCoins(sdk.NewCoin("okb", sdk.NewDec(1))), account.GetCoins())
					}
					return false
				})
		...
		},
		{
			...
			postcheck: func(ctx *sdk.Context, stateDB *types.CommitStateDB) {
				...
				suite.stateDB.ForEachStorageForTest(*ctx, obj, func(key, value ethcmn.Hash) (stop bool) {
					if bytes.Compare(key.Bytes(), mpt.GetStorageByAddressKey(suite.updateAddr.Bytes(), suite.updateKey.Bytes()).Bytes()) == 0 {
						suite.Require().Equal(ethcmn.BytesToHash([]byte{0x2}), value)
					} else if bytes.Compare(key.Bytes(), mpt.GetStorageByAddressKey(suite.updateAddr.Bytes(), suite.insertKey.Bytes()).Bytes()) == 0 {
			...
			postcheck: func(ctx *sdk.Context, stateDB *types.CommitStateDB) {
				...
				suite.stateDB.ForEachStorageForTest(*ctx, obj, func(key, value ethcmn.Hash) (stop bool) {
					if bytes.Compare(key.Bytes(), mpt.GetStorageByAddressKey(suite.updateAddr.Bytes(), suite.updateKey.Bytes()).Bytes()) == 0 {
						...
						suite.Require().Equal(ethcmn.BytesToHash([]byte{0x1}), value)
					} else if bytes.Compare(key.Bytes(), mpt.GetStorageByAddressKey(suite.updateAddr.Bytes(), suite.deleteKey.Bytes()).Bytes()) == 0 {
						suite.Require().Equal(ethcmn.BytesToHash([]byte{0x1}), value)
					} else {
						panic("can not get more key")
					}
