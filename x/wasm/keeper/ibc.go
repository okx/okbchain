...
func (k Keeper) ensureIbcPort(ctx sdk.Context, contractAddr sdk.WasmAddress) (string, error) {
...
func PortIDForContract(addr sdk.WasmAddress) string {
	return portIDPrefix + addr.String()
}

func ContractFromPortID(portID string) (sdk.WasmAddress, error) {
	if !strings.HasPrefix(portID, portIDPrefix) {
		return nil, sdkerrors.Wrapf(types.ErrInvalid, "without prefix")
	}
	return sdk.WasmAddressFromBech32(portID[len(portIDPrefix):])
}
