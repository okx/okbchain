...
// See https://github.com/okx/okbchain/libs/ics/tree/master/spec/ics-004-channel-and-packet-semantics#channel-lifecycle-management
func (k Keeper) OnOpenChannel(
	...
	res, gasUsed, execErr := k.wasmVM.IBCChannelOpen(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
...
//
// There is an open issue with the [cosmos-sdk](https://github.com/okx/okbchain/libs/cosmos-sdk/issues/8334)
...
func (k Keeper) OnConnectChannel(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasUsed, execErr := k.wasmVM.IBCChannelConnect(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
	k.consumeRuntimeGas(ctx, gasUsed)
	if execErr != nil {
		return sdkerrors.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
func (k Keeper) OnCloseChannel(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasUsed, execErr := k.wasmVM.IBCChannelClose(codeInfo.CodeHash, params, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
	k.consumeRuntimeGas(ctx, gasUsed)
	if execErr != nil {
		return sdkerrors.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
// https://github.com/okx/okbchain/libs/ics/tree/master/spec/ics-004-channel-and-packet-semantics#acknowledgement-envelope
//
// For more information see: https://github.com/okx/okbchain/libs/ics/tree/master/spec/ics-004-channel-and-packet-semantics#packet-flow--handling
func (k Keeper) OnRecvPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasUsed, execErr := k.wasmVM.IBCPacketReceive(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
	k.consumeRuntimeGas(ctx, gasUsed)
	if execErr != nil {
...
// contract. The use of the standard acknowledgement envelope is recommended: https://github.com/okx/okbchain/libs/ics/tree/master/spec/ics-004-channel-and-packet-semantics#acknowledgement-envelope
//
...
func (k Keeper) OnAckPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasUsed, execErr := k.wasmVM.IBCPacketAck(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
	k.consumeRuntimeGas(ctx, gasUsed)
	if execErr != nil {
		return sdkerrors.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
...
func (k Keeper) OnTimeoutPacket(
	...
	gas := k.runtimeGasForContract(ctx)
	res, gasUsed, execErr := k.wasmVM.IBCPacketTimeout(codeInfo.CodeHash, env, msg, prefixStore, cosmwasmAPI, querier, ctx.GasMeter(), gas, costJSONDeserialization, uint64(ctx.BlockHeight()))
	k.consumeRuntimeGas(ctx, gasUsed)
	if execErr != nil {
		return sdkerrors.Wrap(types.ErrExecuteFailed, execErr.Error())
	}
