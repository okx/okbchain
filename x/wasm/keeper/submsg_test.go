...
func TestDispatchSubMsgErrorHandling(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"send tokens": {
			submsgID:         5,
			msg:              validBankSend,
			resultAssertions: []assertion{assertReturnedEvents(0), assertGasUsed(90000, 100000)},
		},
		"not enough tokens": {
			submsgID:    6,
			msg:         invalidBankSend,
			...
			resultAssertions: []assertion{assertGasUsed(73000, 82000), assertErrorString("codespace: sdk, code: 5")},
		},
		...
		"out of gas caught with gas limit": {
			...
			resultAssertions: []assertion{assertGasUsed(subGasLimit+71000, subGasLimit+80000), assertErrorString("codespace: sdk, code: 11")},
		},
