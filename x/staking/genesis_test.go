...
func TestInitGenesisLargeValidatorSet(t *testing.T) {
	...
	for i := range validators {
		validators[i] = NewValidator(sdk.ValAddress(Addrs[i]),
			PKs[i], NewDescription(fmt.Sprintf("#%d", i), "", "", ""), types.DefaultDPoSMinSelfDelegation)
...
func TestValidateGenesis(t *testing.T) {
	...
	genValidators[0] = NewValidator(sdk.ValAddress(pk.Address()), pk, generatedValDesc, types.DefaultDPoSMinSelfDelegation)
