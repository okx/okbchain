...
func NewChain(env *Env) *Chain {
	...
	for i := 0; i < 10; i++ {
		chain.acc[i] = apptypes.EthAccount{
			BaseAccount: &auth.BaseAccount{
				Address: env.addr[i],
				Coins:   sdk.Coins{sdk.NewInt64Coin("okb", 1000000)},
			},
	...
	stakingParams := stakingtypes.DefaultDposParams()
	chain.app.StakingKeeper.SetParams(chain.Ctx(), stakingParams)
...
func createTokenSendTx(t *testing.T, chain *Chain, i int) []byte {
	msg := tokentypes.NewMsgTokenSend(chain.addr[i], chain.addr[i+1], sdk.Coins{sdk.NewInt64Coin("okb", 1)})
...
func createFailedTokenSendTx(t *testing.T, chain *Chain, i int) []byte {
	msg := tokentypes.NewMsgTokenSend(chain.addr[i], chain.addr[i+1], sdk.Coins{sdk.NewInt64Coin("okb", 100000000000)})
...
func TestParallelTxs(t *testing.T) {
	...
	tmtypes.UnittestOnlySetMilestoneMercuryHeight(1)
	...
	testCases := []struct {
		...
		},
		{
			...
			[]uint32{161034, 5, 161034, 161034, 161034},
		},
		{
		...
		},
		{
			...
			[]uint32{161034, 161034, 161034, 161034, 161034},
		},
		{
